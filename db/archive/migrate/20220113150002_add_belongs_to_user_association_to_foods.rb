class AddBelongsToUserAssociationToFoods < ActiveRecord::Migration[7.0]
  def up
    ...
    add_reference :foods, :user, index: true, foreign_key: true, null: true
    Food.update_all(user_id: find_user.id) if find_user.present? # rubocop:disable Rails/SkipsModelValidations
