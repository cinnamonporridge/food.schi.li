...
class BackupDatabaseTest < ActiveSupport::TestCase
  test "#call without username, password and host" do
    ...
    expected_command = %r{pg_dump --no-password --dbname [^ ]+ --file /tmp/food/test\.latest\.dump}
  ...
  test "#call with username" do
    ...
    expected_command =
      %r{pg_dump --no-password --username fake_user --dbname [^ ]+ --host fake-host --file /tmp/food/test\.latest\.dump}
