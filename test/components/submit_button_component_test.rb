...
class SubmitButtonComponentTest < ViewComponent::TestCase
  test '#render with new object' do
    render_inline SubmitButtonComponent.new('Add food', Food.new)
  ...
  test '#render with persisted object' do
    render_inline SubmitButtonComponent.new('Update food', foods(:apple))
    assert_selector 'button[type=submit]'
  ...
  test '#render with custom icon' do
    render_inline SubmitButtonComponent.new('Copy thing', :duplicate)
    assert_selector 'button[type=submit]'
